...
var settings_block = document.createElement('div');
    settings_block.className = "template-settings";
...
/*
    ...
    set_settings(template_settings,false);    
    
    $(".template-settings input").on("ifClicked",function(){
        ...
        if(input.attr("name") != 'st_layout_boxed'){
            ...
            if(!input.prop("checked")){
                template_settings[input.attr("name")] = input.val();
            }else{            
                template_settings[input.attr("name")] = 0;
            }
        ...
        if(input.attr("name") === 'st_sb_fixed'){
            if(template_settings.st_sb_fixed == 1){
                template_settings.st_sb_scroll = 1;
            }else{
                template_settings.st_sb_scroll = 0;
            }
        }
        
        if(input.attr("name") === 'st_sb_scroll'){
            if(template_settings.st_sb_scroll == 1 && template_settings.st_layout_boxed == 0){
                template_settings.st_sb_fixed = 1;
            }else if(template_settings.st_sb_scroll == 1 && template_settings.st_layout_boxed == 1){
                template_settings.st_sb_fixed = -1;
            }else if(template_settings.st_sb_scroll == 0 && template_settings.st_layout_boxed == 1){
                template_settings.st_sb_fixed = -1;
            }else{
                template_settings.st_sb_fixed = 0;
            }
        }
        
        if(input.attr("name") === 'st_layout_boxed'){
            if(template_settings.st_layout_boxed == 1){                
                template_settings.st_head_fixed    = -1;
                template_settings.st_sb_fixed      = -1;
                template_settings.st_sb_scroll     = 1;
                
                $("#ts-wallpapers").show();
            }else{
                template_settings.st_head_fixed    = 0;
                template_settings.st_sb_fixed      = 1;
                template_settings.st_sb_scroll     = 1;
        ...
        set_settings(template_settings,input.attr("name"));
        
    });
    
    /* Change template */
    $(".ts-templates a[data-template]").click(function(){
        $(".ts-templates a[data-template]").removeClass("active");
        $(this).addClass("active");
        $("#template").attr("href",$(this).data("template"));
        return false;
    });
    /* END Change template */
    
    /* Change Wallpaper */
    $(".ts-templates a[data-wall]").click(function(){
        $(".ts-templates a[data-wall]").removeClass("active");
        $(this).addClass("active");
    ...
    $(".ts-button").on("click",function(){
        $(".template-settings").toggleClass("active");
    });
    /* End open/hide settings */
});

function set_settings(template_settings,option){
    
    /* Start Header Fixed */
    if(template_settings.st_head_fixed == 1)
    ...
    /* Start Sidebar Fixed */
    if(template_settings.st_sb_scroll == 1){          
    ...
    /* Start Right Sidebar */
    if(template_settings.st_sb_right == 1)
    ...
    if(template_settings.st_sb_custom == 1)
    ...
    if(option && option === 'st_sb_toggled'){
        if(template_settings.st_sb_toggled == 1){
    ...
    if(option === false || option === 'st_layout_boxed' || option === 'st_sb_fixed' || option === 'st_sb_scroll'){        
        for(option in template_settings){
            set_settings_checkbox(option,template_settings[option]);
        }
    }
...
function set_settings_checkbox(name,value){
    ...
    if(name == 'st_layout_boxed'){    
        ...
        $(".template-settings").find("input[name="+name+"]").prop("checked",false).parent("div").removeClass("checked");
        
        var input = $(".template-settings").find("input[name="+name+"][value="+value+"]");
        ...
        var input = $(".template-settings").find("input[name="+name+"]");
